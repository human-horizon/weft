...
export interface Workflow<CurrentCtx = Record<string, never>, InitialCtx = CurrentCtx> {
  ...
  ): Workflow<CurrentCtx & { [K in Name]: AgentResult }, InitialCtx>;

  prompt<Name extends string, S extends z.ZodType>(
    name: Name,
    fn: (ctx: CurrentCtx) => string,
    opts: PromptOpts & { schema: S },
  ): Workflow<CurrentCtx & { [K in Name]: z.infer<S> }, InitialCtx>;

  step<Name extends string, Out>(
    name: Name,
    fn: (ctx: CurrentCtx) => Out | Promise<Out>,
    opts?: StepOpts,
  ): Workflow<CurrentCtx & { [K in Name]: Awaited<Out> }, InitialCtx>;
