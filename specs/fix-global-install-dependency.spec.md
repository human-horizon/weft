...
- [x] `pnpm pack` создаёт tarball.
- [ ] `pnpm add -g ./human-horizon-weft-*.tgz` устанавливается без ошибки `@aws-sdk/token-providers@3.1100.0`.
  **Результат:** ошибка осталась — AWS SDK v3 имеет каскад битых транзитивных зависимостей (`@smithy/node-http-handler@^4.9.13` не существует).
- [ ] `weft --version` работает после глобальной установки tarball.
- [x] Пользователь получает точные команды для публикации.

## Вывод

Проблема не в версии `@earendil-works/pi-*`, а в самом AWS SDK v3: внутри него разъехались версии транзитивных зависимостей (`@aws-sdk/token-providers@3.1100.0` не существует, `@smithy/node-http-handler@^4.9.13` не существует).

Единственный рабочий способ установить weft глобально — `pnpm link --global` из локальной директории (использует lock-файл с зафиксированными версиями).
