...
function cmdRun(runArgs) {
    ...
    console.log(`${dim("⚡ weft run")} ${cyan(filePath)} ${dim(extraArgs.join(" ") || "")}`);
...
function cmdInstall() {
    ...
    const pkg = packageExists
        ? JSON.parse(readFileSync(lorePkgPath, "utf-8"))
        : {
